"""Tests for Failure Analysis API endpoints.
...
class TestAnalyzeEndpoint:
    """Tests for POST /api/v1/failures/analyze."""
    ...
    async def test_analyze_success(self, client):
        ...
        response = await client.post(
            ...
            json={"content": "AssertionError: expected 200, got 500"},
        )
    ...
    async def test_analyze_with_ci_log(self, client):
...
class TestGetSessionEndpoint:
    """Tests for GET /api/v1/failures/sessions/{id}."""

    async def test_get_nonexistent_session(self, client):
        """Non-existent session should return 404."""
        response = await client.get(
            f"/api/v1/failures/sessions/{UUID('00000000-0000-0000-0000-000000000099')}",
        )
        assert response.status_code == 404


class TestListSessionsEndpoint:
    """Tests for GET /api/v1/failures/sessions."""

    async def test_list_sessions(self, client):
        """Listing sessions should return 200 with pagination."""
        response = await client.get("/api/v1/failures/sessions")
        assert response.status_code == 200
        body = response.json()
        assert "data" in body
        assert "meta" in body
    ...
    async def test_list_sessions_pagination(self, client):
        ...
        response = await client.get(
            "/api/v1/failures/sessions?page=1&page_size=10",
        )
        assert response.status_code == 200
